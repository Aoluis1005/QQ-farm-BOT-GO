...
>
> 🏁 **七夕「鹊桥寄情」活动已结束（游戏侧已下线）**：灵露喷洒 / 筑建鹊桥 / 档位奖励 / 送香囊功能为历史实现，仅活动期间可用。
>
...
### 🎋 七夕「鹊桥寄情」（活动已下线）
> 该活动游戏侧已于 2026-08 下线，以下功能为活动期间的历史实现，仅作记录。

| 功能 | 状态 |
|---|---|
| 灵露喷洒（自动触发鹊羽效果，收获鹊羽） | 🏁 已下线（历史实现） |
| 筑建鹊桥（3 档奖励：香囊 / 礼包 / 铭牌） | 🏁 已下线（历史实现） |
| 档位已领标记（前端正确显示可筑档） | 🏁 已下线（历史实现） |
| 送香囊（activity_id=1802, cmd=26, params{friend_gid,count}） | 🏁 已下线（历史实现） |

### 🌧️ 雨落成诗（占位实现 · 待开服抓包）

> ⚠️ **本活动接口尚未真正实现。** 前端面板与后端路由（5 个 `/api/activity/yulu/*` 端点）已搭好框架，但底层游戏 RPC 全部是占位 stub：
> - 物品使用指令（cmd / 目标地块参数）暂抄七夕喷洒协议，**正确性未验证**；
...
| 好友向 采集 / 引雷 / 青蛙 / 乌云 | 🟡 占位 | 编码参考七夕喷洒，真实 cmd 待一锤 |
