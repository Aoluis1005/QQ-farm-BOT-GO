...
> ⚠️ **本活动接口尚未真正实现。** 前端面板与后端路由（5 个 `/api/activity/yulu/*` 端点）已搭好框架，但底层游戏 RPC 全部是占位 stub：
> - 物品使用指令（cmd / 目标地块参数）暂抄既有喷洒协议，**正确性未验证**；
> - 雷电徽章 ID、**气象研究**档位与领奖指令未知，目前返回「待开服抓包」；
> - 顶部 7 瓶库存为**真实读背包**（仅展示物品 ID 与数量），但物品真实用途待开服核对。
>
> 活动预计 **2026-08-26 10:00（北京时间）** 开服，届时通过抓包补全上述协议后即为可用状态。

| 功能 | 状态 | 说明 |
|---|---|---|
| 顶部 7 瓶库存读取 | 🟡 框架就绪 | 现拉背包展示，真实物品 ID 待开服核对 |
| 雷雨召唤 / 百宝惊喜 / 雷纹礼盒（打开） | 🟡 占位 | 走通用 Use 框架，真实 cmd 待抓包 |
| 闪电变异瓶（一键对自己地块） | 🟡 占位 | 地块筛选逻辑已定死，真实效果待验证 |
| 好友向 采集 / 引雷 / 青蛙 / 乌云 | 🟡 占位 | 编码参考既有喷洒，真实 cmd 待一锤 |
| 雷电徽章数量 / 气象研究档位领奖 | 🔴 未实现 | 徽章 id 与档位 RPC 未知，纯 stub |
